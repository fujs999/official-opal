...
bool H460_FeatureStd19::OnSendingOLCGenericInformation(unsigned sessionID, H245_ArrayOf_GenericParameter & param, bool /*isAck*/)
{
  ...
  if (server == NULL) {
    ...
    if (transport == NULL) {
      PTRACE(2, "Session " << sessionID << " has no transport.");
      return false;
    }

    H46019UDPSocket * rtp = dynamic_cast<H46019UDPSocket *>(transport->GetChannel(OpalRTPSession::e_Media)->GetBaseReadChannel());
    if (rtp == NULL) {
...
void H460_FeatureStd19::OnReceiveOLCGenericInformation(unsigned sessionID, const H245_ArrayOf_GenericParameter & params, bool /*isAck*/)
{
  ...
  H46019UDPSocket * rtcp = dynamic_cast<H46019UDPSocket *>(transport->GetChannel(OpalRTPSession::e_Control)->GetBaseReadChannel());
  if (rtcp == NULL) {
