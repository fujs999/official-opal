...
static atomic<OpalIM::MessageID> s_messageIdCounter;

OpalIM::MessageID OpalIM::GetNextMessageId()
{
  return ++s_messageIdCounter;
}
...
PSafePtr<OpalIMContext> OpalIMEndPoint::FindContextForMessageWithLock(OpalIM & im, OpalConnection * conn)
{
  ...
  if (context == NULL) {
    ...
    if (context != NULL) {
      PTRACE_IF(4, !im.m_conversationId.IsEmpty(), "OpalIM\tMatched to/from addresses but did not match ID");
