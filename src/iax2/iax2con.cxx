...
IAX2Connection::IAX2Connection(OpalCall & call,               /* Owner call for connection        */
  ...
  , m_jitterBuffer(OpalJitterBuffer::Create(OpalMediaType::Audio(), OpalJitterBuffer::Init(ep.GetManager().GetJitterParameters(), 8, ep.GetManager().GetMaxRtpPacketSize())))
{  
...
bool IAX2Connection::SetConnected()
{
  ...
  if (m_mediaStreams.IsEmpty()) {
    ...
    m_jitterBuffer->SetDelay(OpalJitterBuffer::Init(endpoint.GetManager().GetJitterParameters(), 8, endpoint.GetManager().GetMaxRtpPacketSize()));
...
void IAX2Connection::ReceivedSoundPacketFromNetwork(IAX2Frame *soundFrame)
{
    ...
    mediaFrame.SetPayload(soundFrame->GetMediaDataPointer(), soundFrame->GetMediaDataSize());
    m_jitterBuffer->WriteData(mediaFrame);
