...
RUN rpm --import http://packages.atrpms.net/RPM-GPG-KEY.atrpms && yum install -y http://dl.atrpms.net/all/atrpms-repo-6-7.el6.x86_64.rpm && yum clean all
...
RUN yum-builddep -y --downloadonly --downloaddir=/var/cache/jenkins/build-deps bbcollab-libopal.spec && yum clean all
